...
import { FileText, DollarSign, Users, TrendingUp, Loader2, LogOut } from 'lucide-react';
import OrdersList from './OrdersList';
import InvoicePreview from './InvoicePreview';
import { ordersApi } from '../services/api';

const Dashboard = ({ shop, onLogout }) => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await ordersApi.getAll(shop, { limit: 50 });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {shop ? (
                    <span className="font-medium text-blue-600">{shop}</span>
                  ) : (
                    'Manage orders and create invoices'
                  )}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  className="text-gray-600 hover:text-gray-900 px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors flex items-center"
